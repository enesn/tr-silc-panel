...
# A list of each panel wave: (label, folder_path, subfolder, separator)
PANELS = [
...
def load_panel(label: str, folder: str, sep: str) -> dict[str, pd.DataFrame]:
    ...
    return {
        "f":  pd.read_csv(f"{folder}/{prefix}{label}_f.csv",  sep=sep),
        "fk": pd.read_csv(f"{folder}/{prefix}{label}_fk.csv", sep=sep),
        "h":  pd.read_csv(f"{folder}/{prefix}{label}_h.csv",  sep=sep),
        "hk": pd.read_csv(f"{folder}/{prefix}{label}_hk.csv", sep=sep),
    }
